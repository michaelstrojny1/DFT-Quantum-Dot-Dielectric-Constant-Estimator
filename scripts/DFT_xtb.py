...
def run_xtb_polarizability(atoms, method="2", charge: int = 0, uhf: int = 0):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # Define a series of attempts with varying settings for robustness (vacuum only)
        if method == "ff":
            # GFN-FF settings for inorganic materials
            attempts = [
                {"alpb": None, "etemp": 300, "iterations": 1000, "timeout": 1800, "use_property": True},
                {"alpb": None, "etemp": 500, "iterations": 1500, "timeout": 2400, "use_property": True},
            ]
        else:
            # Original GFN1/2 settings
            attempts = [
                {"alpb": None,  "etemp": 500,  "iterations": 500,  "timeout": 1200, "use_property": False},
                {"alpb": None,  "etemp": 800,  "iterations": 1200, "timeout": 1800, "use_property": True},
                {"alpb": None,  "etemp": 1000, "iterations": 1500, "timeout": 2400, "use_property": True},
            ]
...
def main():
    ...
    ap.add_argument("--method", choices=["1", "2", "ff"], default="2", help="GFN method (1, 2, or ff for GFN-FF)")
