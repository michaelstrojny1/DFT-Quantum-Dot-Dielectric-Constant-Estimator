...
# Try to import quadratic hex element if available; otherwise, fall back.
try:
    from skfem import ElementHex2 as _ElementHex2
    _HAS_HEX2 = True
...
def main():
    ...
    ap.add_argument("--order", type=int, choices=[1, 2], default=1,
                    help="Finite element order: 1=linear (fast), 2=quadratic (smoother)")
    args = ap.parse_args()
    ...
    if args.order == 2 and not _HAS_HEX2:
        print("[WARN] ElementHex2 not available in this scikit-fem version; falling back to ElementHex1.", file=sys.stderr)
        args.order = 1
    e = _ElementHex2() if (args.order == 2 and _HAS_HEX2) else ElementHex1()
    basis = Basis(m, e)

    epsfun = build_epsfun(Lx, Ly, Lz, hbn_bot, hbn_mid, hbn_top, eps_hbn, centers, R, shells, eps_vac)
    # Project coefficient onto the chosen basis to support both linear and quadratic elements
    eps_dof = basis.project(epsfun)
    eps_q = basis.interpolate(eps_dof)
    ...
    # Interpolate phi to the exact graphene plane (zg), rather than snapping to nearest z layer
    node_z = m.p[2]
    uniq_z = np.unique(np.round(node_z, 15))

    nxn, nyn = args.nx + 1, args.ny + 1

    def layer_nodes_for(k: int):
        start = k * (nxn * nyn)
        return np.arange(start, start + nxn * nyn)

    # Find bracketing layers k0 <= zg <= k1
    if zg <= uniq_z[0] + 1e-15:
        k0 = k1 = 0
        t = 0.0
    elif zg >= uniq_z[-1] - 1e-15:
        k0 = k1 = len(uniq_z) - 1
        t = 0.0
    else:
        k1 = int(np.searchsorted(uniq_z, zg, side="right"))
        k0 = k1 - 1
        z0, z1 = uniq_z[k0], uniq_z[k1]
        t = float((zg - z0) / (z1 - z0))

    ln0 = layer_nodes_for(k0)
    ln1 = layer_nodes_for(k1)

    # x,y coordinates are identical across layers in a tensor mesh; read from ln0
    px = m.p[0, ln0].reshape((nyn, nxn))
    py = m.p[1, ln0].reshape((nyn, nxn))

    phi0 = phi[ln0]
    phi1 = phi[ln1]
    pphi = ((1.0 - t) * phi0 + t * phi1).reshape((nyn, nxn))
    ...
    with open(outdir / "summary.txt", "w") as f:
        ...
        f.write(f"phi_graphene_min,phi_graphene_max,{pphi.min():.6e},{pphi.max():.6e}\n")
